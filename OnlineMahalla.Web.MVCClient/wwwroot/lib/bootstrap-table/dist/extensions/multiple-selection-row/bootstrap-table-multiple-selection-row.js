...
!function ($) {
    ...
    var onRowClick = function (e) {
        ...
        if (window.event.button === 0) {
            ...
            if (window.event.shiftKey) {
                selectRowsBetweenIndexes([that.bootstrapTable("getOptions").multipleSelectRowLastselectedid.rowIndex, e.currentTarget.rowIndex], that)
            }
        }
    };
    ...
    var toggleRow = function (row, that, clearAll, useShift) {
        if (clearAll) {
            row = $(row);
            that.bootstrapTable("getOptions").multipleSelectRowLastselectedid = undefined;
            row.removeClass(that.bootstrapTable("getOptions").multipleSelectRowCssClass);
            that.bootstrapTable("uncheck", row.data("index"));    
        } else {
            that.bootstrapTable("getOptions").multipleSelectRowLastselectedid = row;
            row = $(row);
            if (useShift) {
    ...
    $.extend($.fn.bootstrapTable.defaults, {
        ...
        multipleSelectRowLastselectedid: undefined,
    ...
    BootstrapTable.prototype.init = function () {
        if (this.options.multipleSelectRow) {
            ...
            this.options.multipleSelectRowLastselectedid = undefined;
